...
class VSM(RecMixin, BaseRecommenderModel):
    ...
    def __init__(self, data, config, params, *args, **kwargs):
        ...
        if self._user_profile_type == "tfidf":
            self._tfidf_obj = TFIDF(self._data.side_information_data.kahfm_feature_map)
        ...
        if self._item_profile_type == "tfidf":
            ...
            self._i_item_feature_dict = {
                i_item: {self._data.public_features[feature]: self._tfidf[item].get(feature, 0)
                         for feature in self._data.side_information_data.kahfm_feature_map[item]}
                for item, i_item in self._data.public_items.items() if
                self._data.side_information_data.kahfm_feature_map.get(item, None)}
            ...
            self._i_item_feature_dict = {i_item: [self._data.public_features[feature] for feature
                                                  in self._data.side_information_data.kahfm_feature_map[item]]
                                         for item, i_item in self._data.public_items.items() if
                                         self._data.side_information_data.kahfm_feature_map.get(item, None)}
    ...
    def compute_binary_profile(self, user_items_dict: t.Dict):
        ...
        for item in user_items_dict.keys():
            for feature in self._data.side_information_data.kahfm_feature_map.get(item, []):
